...
export class PatForm {
  constructor(form) {
    ...
    form.elements.drug_type.addEventListener("change", () =>
      this.#reactToDrugChange(),
    );
    form.elements.drug_hours.addEventListener("change", () =>
      this.#reactToDrugHoursChange(),
    );

    form.elements.licensed_therapists_drug.addEventListener("input", (e) =>
      this.#reactToTherapistChange(
        "licensed_therapists_hours",
        e.currentTarget.value,
      ),
    );
    form.elements.unlicensed_therapists_drug.addEventListener("input", (e) =>
      this.#reactToTherapistChange(
        "unlicensed_therapists_hours",
        e.currentTarget.value,
      ),
    );
    ...
    form.elements.different_sessions.addEventListener("change", () =>
      this.#reactToPhysicianChange(),
    );
    form.elements.drug_sessions.addEventListener("input", () =>
      this.#reactToPhysicianChange(),
    );
    ...
    this.#reactToTherapistChange(
      "licensed_therapists_hours",
      form.elements.licensed_therapists_drug.value,
    );
    this.#reactToTherapistChange(
      "unlicensed_therapists_hours",
      form.elements.unlicensed_therapists_drug.value,
    );
    this.#reactToPhysicianChange();
  }
  ...
  #reactToTherapistChange(selectorBase, count) {
    ...
    if (delta > 0) {
      for (let i = 0; i < delta; i++) {
        ...
        clone.querySelector(".number").textContent =
          `#${count - delta + i + 1}`;
  ...
  #reactToPhysicianChange() {
    ...
    const hoursContainer = this.form.querySelector(
      ".container.physician_hours",
    );
    const sessionsContainer = this.form.querySelector(
      ".container.physician_hours > .physician_sessions",
    );
    const currentHours = Array.from(
      sessionsContainer.querySelectorAll("slider-input"),
    ).map((el) => el.value);
    while (sessionsContainer.lastElementChild)
      sessionsContainer.removeChild(sessionsContainer.lastElementChild);
    ...
    this.form.querySelector(".different_sessions_wrapper").style.display =
      sessions === 1 ? "none" : null;
    ...
    if (diffHours) {
      for (let i = 0; i < sessions; i++) {
        const clone = this.form
          .querySelector("template.physician_hours_multi")
        ...
        clone
          ...
          .setAttribute(
            "value",
            Math.min(drugHours, currentHours[i] ?? drugHours),
          );
        sessionsContainer.appendChild(clone);
      }
    } else {
      const clone = this.form
        .querySelector("template.physician_hours_single")
        .content.cloneNode(true);
      clone.querySelector("slider-input").setAttribute("max", drugHours);
      clone
        .querySelector("slider-input")
        .setAttribute(
          "value",
          Math.min(drugHours, currentHours[0] ?? drugHours),
        );
      sessionsContainer.appendChild(clone);
    }
  }
  ...
  get drug() {
    const getMultipleValueSum = (name) => {
      ...
      if (!el) {
        ...
        return [...el].reduce((acc, el) => acc + Number(el.value), 0);
      }
    };
    const sessions = this.form.elements.drug_sessions.value;
    return {
      ...
      licensedHours:
        sessions * getMultipleValueSum("licensed_therapists_hours"),
      unlicensedHours:
        sessions * getMultipleValueSum("unlicensed_therapists_hours"),
      physicianHours: this.form.elements.different_sessions.checked
        ? getMultipleValueSum("physician_hours")
        : (this.form.elements.physician_hours?.value ?? 0) *
          this.form.elements.drug_sessions.value,
